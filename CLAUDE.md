...
`<Name>` comes from `GAME_TABS` (`Hunt`, `Race`, `Trivia`, `Sushi`, `Arcade`). **Every other top-level name in a module must be prefixed** — `hunt*`/`HUNT_`, `race*`/`RACE_`, `trivia*`/`TRIVIA_`, `arcade*`/`ARCADE_`/`DEER_`, `ck*`, `route*`/`ROUTE_` — because there is no scope isolation. Adding a game means: new `game-*.js`, a `<script>` tag, and a `GAME_TABS` entry.
